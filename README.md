...
2. **Hero** — à l'arrivée, une **boucle vidéo « vivante »** (voilage ensoleillé, ombres qui bougent, montée en boucle sans couture par palindrome) : rien n'est figé. Au premier scroll, **fondu quasi invisible** vers le travelling scrubé du couloir d'hôtel particulier (les deux partagent des voilages → raccord). Séquence de 150 images JPEG dessinées dans un canvas, inertie, fragments de phrases calés sur la progression.
3. **La bienvenue** — la transition signature : au cœur du travelling qui continue de tourner, la lumière baisse et un grand titre s'allume au centre — « Bienvenue · **La Maison vous ouvre ses portes.** » — entièrement en `mix-blend-mode: difference` (couleur **inversée en direct** sur le hall) ; puis le couloir se dissout en fondu enchaîné vers la promenade.
4. **Le manifeste** — trois phrases une à une, posées sur une **promenade scrubée à travers trois pièces distinctes** (salon céladon, salon marine à l'art coloré, séjour au couchant), chaque phrase à un endroit différent de l'écran. Les accents (ex-jaunes) passent en **couleur inversée** sur la vidéo. La dernière — « Entrez » — se **liquéfie** (effet *gooey*, seuil d'alpha SVG) en « dans la couleur » avant de révéler la pièce suivante (même section sticky, couture invisible).
...
10. **Le rideau** — tout le site se lève et découvre le footer émeraude qui attendait dessous ; une **traînée de souris « halftone »** (trame de points en WebGL, d'après un composant 21st.dev porté en vanilla) suit le curseur sur le fond vert.
...
- Micro-effets inspirés du catalogue [21st.dev](https://21st.dev) réécrits en vanilla : morph liquide *gooey* du titre (filtre de seuil SVG + flou piloté au scroll), traînée de souris *halftone* en WebGL sur le footer (`js/halftone-trail.js`), lignes de texte qui montent sous masque (SplitText), images éditoriales révélées en rideau (`clip-path`).
- **Boucle idle « vivante »** : `assets/video/hero-idle.mp4` (palindrome, boucle sans couture) autoplay muet, fondu vers le canvas au premier scroll.
...
assets/video/       hero-idle.mp4 (boucle vivante du hero)
