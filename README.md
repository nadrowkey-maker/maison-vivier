...
> Toutes les images et vidéos proviennent de [Pexels](https://www.pexels.com) (licence gratuite, usage commercial autorisé).
...
- Les « vidéos » scrubées sont des **séquences d'images JPEG** (3 × 150 frames, extraites avec ffmpeg) dessinées dans un `<canvas>` en cover manuel, `devicePixelRatio` plafonné à 1,5, redraw uniquement quand l'index change. Le scroll fixe une cible, la frame affichée la rattrape (`current += (target - current) × 0.075`).
- Micro-effets inspirés du catalogue [21st.dev](https://21st.dev) réécrits en vanilla : brillance or balayée sur le mot des portes (`background-clip: text`), lignes de texte qui montent sous masque (SplitText), images éditoriales révélées en rideau (`clip-path`).
